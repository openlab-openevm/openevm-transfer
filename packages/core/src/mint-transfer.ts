...
export async function neonTransferMintTransaction<W extends Provider, TxResult extends TransactionResult>(params: NeonMintTxParams<W, TxResult>): Promise<Transaction> {
  ...
  const associatedTokenAddress = getAssociatedTokenAddressSync(new PublicKey(splToken.address_spl), solanaWallet, false, TOKEN_2022_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID);
 ...
 * @param {PublicKey} [params.programId=TOKEN_2022_PROGRAM_ID] - The public key of the token program (default: `TOKEN_2022_PROGRAM_ID`) from '@solana/spl-token' library.
...
export async function createWrapSOLTransaction({
  ...
  const associatedToken = getAssociatedTokenAddressSync(tokenMint, solanaWallet,false, TOKEN_2022_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID);
  ...
  instructions.push(createSyncNativeInstruction(associatedToken, TOKEN_2022_PROGRAM_ID));
