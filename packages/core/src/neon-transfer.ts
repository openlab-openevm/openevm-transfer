...
export async function solanaNEONTransferTransaction({
  ...
  const associatedTokenAddress = getAssociatedTokenAddressSync(new PublicKey(neonToken.address_spl), solanaWallet, false, TOKEN_2022_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID);
  const transaction = new Transaction({ feePayer: solanaWallet });

  transaction.add(createApproveInstruction(associatedTokenAddress, balanceAddress, solanaWallet, fullAmount,[], TOKEN_2022_PROGRAM_ID));
...
export function createNeonDepositToBalanceInstruction({
  ...
  const poolAddress = getAssociatedTokenAddressSync(tokenMint, depositWallet, true, TOKEN_2022_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID);
  const keys = [
...
export function createNeonDepositInstruction(solanaWallet: PublicKey, neonPDAWallet: PublicKey, depositWallet: PublicKey, neonWallet: string, neonEvmProgram: PublicKey, neonTokenMint: PublicKey, serviceWallet?: PublicKey): TransactionInstruction {
  const solanaAssociatedTokenAddress = getAssociatedTokenAddressSync(neonTokenMint, solanaWallet, false, TOKEN_2022_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID);
  const poolKey = getAssociatedTokenAddressSync(neonTokenMint, depositWallet, true, TOKEN_2022_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID);
  const keys = [
...
export function createNeonTransferInstruction({
  ...
  const from = getAssociatedTokenAddressSync(neonTokenMint, solanaWallet, true, TOKEN_2022_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID);
  const to = getAssociatedTokenAddressSync(neonTokenMint, serviceWallet, true, TOKEN_2022_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID);
  ...
  return new TransactionInstruction({ programId: TOKEN_2022_PROGRAM_ID, keys, data });
}
