...
import {TOKEN_2022_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID, getAssociatedTokenAddressSync,} from 'open-token-web3';
...
import { TokenBalance, TransactionFunctionsMap, TransferDirection, TransferSignature } from './models';
...
const networkUrls = [{
  id: 286022980,
  token: 'NEON',
  solana: 'https://api.mainnet.openverse.network',
  neonProxy: 'https://rpc1.istchain.org'
}, {
  id: 285022927,
  token: 'SOL',
  solana: 'https://api.mainnet.openverse.network',
...
function NeonTransferApp() {
    ...
    if (splToken) {
      switch (splToken.symbol) {
        ...
        case 'wSOL': {
          ...
          const associatedToken = getAssociatedTokenAddressSync(address, solanaWallet.publicKey,false, TOKEN_2022_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID);
  ...
  const handleSolanaTransaction = useCallback(
    async (transactionFunction: () => Promise<any>, solanaSigner: Signer) => {
      try {
    ...
    [connection, setSignature]
  );

  const { wallet, connected, publicKey:solanaPubKey, signTransaction,connect,select} = useWallet();
  ...
  /*const handleSubmit = useCallback(async () => {
    if (token && splToken) {
      ...
      let transactionFunctions: TransactionFunctionsMap;
      ...
      if (transfer.direction === 'solana') {
        ...
        let solana = '';
        if (!['SOL', 'NEON'].includes(splToken.symbol) && !(await connection.getAccountInfo(associatedToken))) {
  ...
  }, [token, splToken, neonProgram, chainId, solanaWallet, walletSigner, networkTokenMint, proxyApi, transfer.direction, getTokenBalance, getWalletBalance, amount, connection, ethersProvider, neonWallet, handleSolanaTransaction]);*/
  const handleSubmit = useCallback(async () => {
    if (token && splToken) {
      setSubmitDisable(true);
      const solanaSigner: Signer = toSigner(solanaWallet); 
      
      if (!connected || !solanaPubKey || !signTransaction) {
        //alert('请连接钱包');
        connect().catch(() => {
        });
        return;
      }
      console.log('--solanaPubKey:', solanaPubKey.toBase58());

      let transactionFunctions: TransactionFunctionsMap;

      if (transfer.direction === 'solana') {
        console.log("--transfer.direction: solana");
        transactionFunctions = {
          NEON: () =>
            solanaNEONTransferTransaction({
              solanaWallet: solanaPubKey,
              neonWallet: neonWallet.address,
              neonEvmProgram: neonProgram,
              neonTokenMint: networkTokenMint,
              token: splToken,
        ...
        const transactionFunction = transactionFunctions[splToken.symbol as keyof typeof transactionFunctions] || transactionFunctions.DEFAULT;
        const transactions = await transactionFunction();
        //await handleSolanaTransaction(transactionFunction, solanaSigner);
         // 发送交易并等待确认
        ...
        console.log("--transfer.direction: neon");
        const mintPubkey = new PublicKey(splToken.address_spl);
        const associatedToken = getAssociatedTokenAddressSync(mintPubkey, solanaWallet.publicKey,false, TOKEN_2022_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID);
        console.log("--associatedToken:", associatedToken.toBase58());
        //Need to create associated token account for wSOL and other SPL tokens
        let solana = '';
        console.log('--splToken.symbol:', splToken.symbol);
        if (!['SOL', 'NEON'].includes(splToken.symbol) && !(await connection.getAccountInfo(associatedToken))) {
          console.log("--begin to createAssociatedTokenAccountTransaction");
          const transaction = createAssociatedTokenAccountTransaction({
            solanaWallet: solanaWallet.publicKey,
            tokenMint: mintPubkey,
            associatedToken,
        ...
        console.log("--createAssociatedTokenAccountTransaction ended..");
        console.log("--neonWallet.address:", neonWallet.address);
        console.log("--solanaWallet.publicKey:", solanaWallet.publicKey.toBase58());
        ...
        transactionFunctions = {
          NEON: () =>
            neonNeonTransactionEthers({
              provider: ethersProvider,
              from: neonWallet.address,
              to: "0x2936b9F662572D81553ac8c48A8afEb3917d0469",//NEON_TRANSFER_CONTRACT_DEVNET,
              solanaWallet: solanaWallet.publicKey,
              amount,
            }),
          SOL: () =>
        ...
        const signWithMetaMask = async (transaction: any) => {
          if (!window.ethereum) {
        ...
        //const neon = await sendNeonTransaction(transaction, neonWallet);
        const neon = await signWithMetaMask(transaction);
        setSignature({ solana, neon });
      }
