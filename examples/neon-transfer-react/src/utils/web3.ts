import { erc20Abi, NEON_TOKEN_MINT_DECIMALS, SPLToken } from 'token-transfer-core-2022';
...
export async function splTokenBalance(connection: Connection, walletPubkey: PublicKey, token: SPLToken): Promise<TokenAmount> {
  ...
  const assocTokenAccountAddress = await getAssociatedTokenAddressSync(mintAccount, walletPubkey,false,
    TOKEN_2022_PROGRAM_ID,
    ASSOCIATED_TOKEN_PROGRAM_ID
  );
  console.log("--splTokenBalance:", mintAccount.toBase58(), " ][ ",assocTokenAccountAddress.toBase58()," ][ ", walletPubkey.toBase58(), " ]");
